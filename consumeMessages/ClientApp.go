...
func consumer(topic string) {
	...
	// assignor.AssignPartitions(topic, partitions)

	// c.SetRebalanceCallback(onRebalance)
	...
	// as there is no commitSync() in this library,
	// use sync.WaitGroup to use the async CommitMessage() as synchronous commit
...
func handleLastcommit(c *kafka.Consumer, record *kafka.Message, wg *sync.WaitGroup) {
	// this will commit in any case the last offset synchronously as we use wg
	// no matter if we re-commit an offset as in this case kafka will ignore it
	go func() {
		_, err := c.CommitMessage(record)
		if err != nil {
			log.Printf("Failed to commit offset: %s\n", err)
			// Handle the error accordingly
			wg.Done()
		}
		log.Printf("Last offset commited ")
		wg.Done()
	}()
}
