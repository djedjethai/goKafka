...
func producer(topic string) {
	...
	msg := Record{"myname", 56}
	addr := Address{"myStreet", "NewYork"}

	// the optional Go fullyQualifiedName("main.Record") assert the msg's FQN
	payloadMsg, err := s.SerializeRecordName(&msg, "main.Record")
	if err != nil {
	...
	for {
		...
		err = producer.Produce(addrOK, nil)
		if err != nil {
			fmt.Printf("Failed to produce message: %v\n", err)
		}
